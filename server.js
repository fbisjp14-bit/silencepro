...
// Fila simples em memória: garante que o Render processe apenas 1 vídeo por vez.
let processQueue = Promise.resolve();
let activeProcessCount = 0;
let queuedProcessCount = 0;

function enqueueProcess(task) {
  queuedProcessCount += 1;
  const run = async () => {
    queuedProcessCount -= 1;
...
app.post('/process', upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    const data = await enqueueProcess(async () => {
      ...
      return {
        ...
        queue: { active: activeProcessCount, waiting: queuedProcessCount }
      };
    });
...
app.get('/download/:file', (req, res) => {
  ...
  let deleted = false;
  function deleteAfterDownload() {
    if (deleted) return;
    deleted = true;
    fs.unlink(full, (unlinkErr) => {
      if (unlinkErr && unlinkErr.code !== 'ENOENT') {
        console.error('Erro ao apagar arquivo baixado:', unlinkErr.message || unlinkErr);
      }
    });
  }
  ...
  res.download(full, file, (err) => {
    if (err) {
      deleted = true;
      console.error('Erro no download:', err.message || err);
      return;
    }
  });

  res.on('finish', deleteAfterDownload);
  res.on('close', () => {
