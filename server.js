...
const DEFAULT_ALLOWED_ORIGINS = ['https://silencepro.pages.dev', 'https://silencepro-sqo1.onrender.com'];
...
function getRenderSettings(mode) {
  const value = String(mode || 'turbo').toLowerCase();

  if (value === 'quality') {
    return {
      mode: 'quality',
      label: 'Qualidade Alta',
      preset: process.env.FFMPEG_QUALITY_PRESET || FFMPEG_PRESET,
      crf: process.env.FFMPEG_QUALITY_CRF || FFMPEG_CRF,
      audioBitrate: '192k',
      fps: null,
      scale: null
    };
  ...
  if (value === 'fast') {
    return {
      ...
      label: 'Rápido',
      preset: 'ultrafast',
      crf: process.env.FFMPEG_FAST_CRF || '28',
      audioBitrate: '128k',
      fps: '30',
      scale: "scale=w=if(gt(iw\\,ih)\\,960\\,-2):h=if(gt(iw\\,ih)\\,-2\\,960)"
    };
  }

  return {
    mode: 'turbo',
    label: 'Relâmpago Keyframe',
    preset: 'ultrafast',
    crf: process.env.FFMPEG_TURBO_CRF || '32',
    audioBitrate: '96k',
    fps: '30',
    scale: "scale=w=if(gt(iw\\,ih)\\,720\\,-2):h=if(gt(iw\\,ih)\\,-2\\,720)"
  };
}
...
async function getVideoKeyframes(inputPath) {
  ...
  const unique = [...new Set(times.map(t => Math.max(0, Number(t.toFixed(3)))))].sort((a, b) => a - b);
  if (!unique.length || unique[0] > 0.05) unique.unshift(0);
  return unique;
}

function firstKeyframeAtOrAfter(keyframes, time) {
  const t = Math.max(0, Number(time) || 0);
  for (const k of keyframes) {
    if (k >= t - 0.001) return k;
  }
  return null;
}

function lastKeyframeAtOrBefore(keyframes, time) {
  const t = Math.max(0, Number(time) || 0);
  let best = null;
  for (const k of keyframes) {
    if (k <= t + 0.001) best = k;
...
function alignCutsToKeyframes(cuts, keyframes, duration) {
  const total = Math.max(0, Number(duration) || 0);
  const aligned = [];

  for (const c of cuts) {
    const rawStart = Math.max(0, Math.min(total, Number(c.start) || 0));
    const rawEnd = Math.max(0, Math.min(total, Number(c.end) || 0));
    if (rawEnd <= rawStart) continue;

    // Para COPY seguro:
    // remove apenas a parte do silêncio que cabe entre keyframes.
    // Assim o vídeo não começa no meio de GOP e não repete cena/áudio.
    const start = firstKeyframeAtOrAfter(keyframes, rawStart);
    const end = lastKeyframeAtOrBefore(keyframes, rawEnd);

    if (start === null || end === null) continue;
    if (end - start >= 0.08) aligned.push({ start, end });
  }
...
async function renderKeyframeCopy(inputPath, outputPath, cuts, duration) {
  const keyframes = await getVideoKeyframes(inputPath);
  const safeCuts = alignCutsToKeyframes(cuts, keyframes, duration);

  if (!safeCuts.length) {
    // Se não existe corte seguro entre keyframes, entrega cópia rápida sem bugar.
    // Isso é melhor do que repetir cena/áudio.
    await run('ffmpeg', [
    ...
    return { appliedCutCount: 0, safeCutCount: 0, keyframeCount: keyframes.length };
  }

  await renderCopyConcat(inputPath, outputPath, safeCuts, duration);

  return {
    appliedCutCount: safeCuts.length,
    safeCutCount: safeCuts.length,
...
async function renderCopyConcat(inputPath, outputPath, cuts, duration) {
  ...
  try {
    // Vídeo em COPY para manter velocidade.
    // Áudio recodificado para AAC para corrigir timestamp/gagueira.
    await run('ffmpeg', [
...
async function renderPreciseSelect(inputPath, outputPath, cuts, duration, outputMode = 'turbo') {
  ...
  filter += `concat=n=${keep.length}:v=1:a=1[vjoin][acat];`;

  const videoFilters = [];
  if (renderSettings.fps) videoFilters.push('fps=' + renderSettings.fps);
  if (renderSettings.scale) videoFilters.push(renderSettings.scale);
  filter += `[vjoin]${videoFilters.length ? videoFilters.join(',') : 'null'}[vcat];`;
  ...
  try {
    await run('ffmpeg', [
      ...
      '-map', '[vcat]', '-map', '[a]',
      '-c:v', 'libx264', '-preset', renderSettings.preset, '-crf', String(renderSettings.crf),
      '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', renderSettings.audioBitrate, '-ar', '48000',
...
async function renderTurboSelect(inputPath, outputPath, cuts, duration, outputMode = 'turbo') {
  // Caminho mais rápido para Render:
  // usa select/aselect em vez de dezenas/centenas de trims + concat.
  // É menos pesado para vídeos longos e arquivos grandes.
  const keep = buildKeepRanges(duration, cuts);
  if (!keep.length) throw new Error('O corte ficou agressivo demais e removeria todo o vídeo.');

  const renderSettings = getRenderSettings(outputMode);
  const expr = keep.map(r => {
    const start = Math.max(0, r.start).toFixed(3);
    const end = Math.max(0, r.end).toFixed(3);
    return `between(t\\,${start}\\,${end})`;
  }).join('+') || '1';

  const videoFilters = [`select='${expr}'`, 'setpts=N/FRAME_RATE/TB'];
  if (renderSettings.fps) videoFilters.push('fps=' + renderSettings.fps);
  if (renderSettings.scale) videoFilters.push(renderSettings.scale);

  const filter = `[0:v]${videoFilters.join(',')}[vcat];` +
    `[0:a]aselect='${expr}',asetpts=N/SR/TB,aresample=async=1:first_pts=0,alimiter=limit=0.98[a]`;
  ...
  await run('ffmpeg', [
    ...
    '-c:v', 'libx264', '-preset', renderSettings.preset, '-crf', String(renderSettings.crf),
    '-pix_fmt', 'yuv420p',
    '-c:a', 'aac', '-b:a', renderSettings.audioBitrate, '-ar', '44100',
    '-movflags', '+faststart',
...
async function renderWithCuts(inputPath, outputPath, cuts, duration, outputMode = 'turbo') {
  if (!cuts.length) {
  ...
  const settings = getRenderSettings(outputMode);

  if (settings.mode === 'turbo') {
    // Relâmpago Keyframe:
    // mantém velocidade com vídeo em copy, mas só corta em keyframes seguros.
    // Isso evita cena repetindo, áudio gaguejando e trecho voltando.
    try {
      const keyframeResult = await renderKeyframeCopy(inputPath, outputPath, cuts, duration);
      return {
        mode: 'relampago-keyframe-copy',
        outputMode: settings.label,
        appliedCutCount: keyframeResult.appliedCutCount,
        keyframeCount: keyframeResult.keyframeCount
      };
    } catch (e) {
      fsp.unlink(outputPath).catch(() => {});
      await renderTurboSelect(inputPath, outputPath, cuts, duration, outputMode);
      return { mode: 'relampago-keyframe-fallback-estavel', outputMode: settings.label };
    }
  }

  if (settings.mode === 'fast') {
    await renderTurboSelect(inputPath, outputPath, cuts, duration, outputMode);
    return { mode: 'rapido-select', outputMode: settings.label };
  }

  await renderPreciseSelect(inputPath, outputPath, cuts, duration, outputMode);
  return { mode: 'precise-anti-gago-anti-chiado-' + settings.mode, outputMode: settings.label };
}
...
app.post('/process', upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    const outputMode = String(req.body.renderMode || 'turbo').toLowerCase();
    ...
    const renderResult = await renderWithCuts(inputPath, outputPath, cuts, info.duration, outputMode);
    ...
    res.json({
      ...
      cutCount: renderResult && Number.isFinite(renderResult.appliedCutCount) ? renderResult.appliedCutCount : cuts.length,
      detectedCutCount: cuts.length,
      renderMode: renderResult && renderResult.mode ? renderResult.mode : 'ok',
      outputMode: renderResult && renderResult.outputMode ? renderResult.outputMode : getRenderSettings(outputMode).label
    });
  } catch (err) {
