...
function runWithProgress(command, args, onProgress, options = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, { ...options, windowsHide: true });
    let stdout = '';
    let stderr = '';
...
async function renderPreciseSelect(inputPath, outputPath, cuts, duration, onProgress) {
  ...
  try {
    await runWithProgress('ffmpeg', [
      ...
      '-movflags', '+faststart', '-max_muxing_queue_size', '2048', '-threads', '0',
      '-progress', 'pipe:1', '-nostats', outputPath
    ], (seconds) => {
      if (typeof onProgress === 'function') {
        const expected = Math.max(0.1, finalSecondsAfterCuts(duration, cuts));
        const pct = 30 + Math.min(64, Math.round((seconds / expected) * 64));
        onProgress(pct, 'Renderizando vídeo...');
      }
    });
  } finally {
...
async function renderWithCuts(inputPath, outputPath, cuts, duration, onProgress) {
  if (!cuts.length) {
  ...
  await renderPreciseSelect(inputPath, outputPath, cuts, duration, onProgress);
...
function publicJob(job) {
  ...
  return {
    ...
    originalSeconds: job.originalSeconds || null,
    finalSeconds: job.finalSeconds || null,
    reductionPercent: job.reductionPercent ?? null,
    silenceCount: job.silenceCount ?? null,
    cutCount: job.cutCount ?? null,
    renderMode: job.renderMode || null
  };
}

async function processJob(job, file, body) {
  const inputPath = file && file.path;
  let outputPath = null;
  try {
    if (!file) throw new Error('Nenhum vídeo foi enviado.');
    setJob(job, { status: 'processing', percent: 3, stage: 'Lendo arquivo...' });
    const threshold = Math.max(-80, Math.min(-10, Number(body.threshold || -30)));
    const minSilence = Math.max(0.01, Math.min(3, Number(body.duration || 0.10)));
    const padding = Math.max(0, Math.min(0.5, Number(body.padding || 0.05)));
    ...
    setJob(job, { percent: 10, stage: 'Detectando silêncios...' });

    const detected = await detectSilences(inputPath, threshold, minSilence);
    setJob(job, { percent: 24, stage: 'Calculando cortes...' });
    ...
    const outputName = 'SilencePro_' + safeBaseName(file.originalname) + '_MP4_limpo_' + crypto.randomUUID().slice(0, 8) + '.mp4';
    outputPath = path.join(OUTPUT_DIR, outputName);
    setJob(job, { percent: 30, stage: 'Renderizando vídeo...' });
    const renderResult = await renderWithCuts(inputPath, outputPath, cuts, info.duration, (percent, stage) => setJob(job, { percent, stage }));

    setJob(job, { percent: 96, stage: 'Finalizando arquivo...' });
    const outInfo = await probe(outputPath).catch(() => null);
    ...
    setJob(job, {
      status: 'done', percent: 100, stage: 'Pronto para baixar!', fileName: outputName,
      downloadUrl: '/download/' + encodeURIComponent(outputName),
      originalSeconds: info.duration.toFixed(1), finalSeconds: realFinalSeconds.toFixed(1), reductionPercent,
      silenceCount: detected.silences.length + (detected.openSilenceStart !== null ? 1 : 0),
      cutCount: cuts.length, renderMode: renderResult && renderResult.mode ? renderResult.mode : 'ok'
    });
  } catch (err) {
    if (outputPath) fsp.unlink(outputPath).catch(() => {});
    setJob(job, { status: 'error', percent: Math.max(1, job.percent || 1), stage: 'Erro no processamento', error: err.message || 'Erro ao processar vídeo.' });
  } finally {
...
app.post('/process', upload.single('video'), async (req, res) => {
  try {
    ...
    const job = {
      ...
      status: 'queued',
      percent: 1,
      stage: 'Arquivo recebido. Iniciando...',
      createdAt: Date.now(),
      updatedAt: Date.now()
    };
    jobs.set(jobId, job);
    const file = req.file;
    const body = { ...req.body };
    setImmediate(() => processJob(job, file, body));
    res.json({ ok: true, jobId, statusUrl: '/status/' + encodeURIComponent(jobId), percent: 1, stage: job.stage });
  } catch (err) {
...
app.get('/status/:id', (req, res) => {
  ...
  if (!job) return res.status(404).json({ ok: false, error: 'Processamento não encontrado ou expirado.' });
