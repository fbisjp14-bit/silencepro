...
let ACTIVE_JOB_STARTED_AT = 0;
let ACTIVE_JOB_PROCESS = null;
...
function run(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('error', err => {
      ...
      finished = true;
      clearTimeout(timer);
      if (ACTIVE_JOB_PROCESS === child) ACTIVE_JOB_PROCESS = null;
      reject(err);
    });
...
function enqueueJob(job, task) {
  ...
  updateJob(job, {
    status: 'queued',
    progress: 5,
    message: pos > 1 ? `Aguardando finalizar o vídeo anterior` : 'Aguardando processamento...'
  });
  drainQueue();
}

async function drainQueue() {
  if (activeJob && ACTIVE_JOB_STARTED_AT && Date.now() - ACTIVE_JOB_STARTED_AT > MAX_PROCESS_MS + 30000) {
    const oldJob = JOBS.get(activeJob);
    if (oldJob) {
      updateJob(oldJob, {
        status: 'error',
        progress: 100,
        message: 'Processamento anterior encerrado',
        error: 'O processamento anterior demorou demais e foi encerrado para liberar a fila.'
      });
      logJob(oldJob, 'Processamento antigo encerrado para liberar a fila.', 'error');
    }
    if (ACTIVE_JOB_PROCESS) {
      try { ACTIVE_JOB_PROCESS.kill('SIGKILL'); } catch (e) {}
      ACTIVE_JOB_PROCESS = null;
    }
    activeJob = null;
    ACTIVE_JOB_STARTED_AT = 0;
  }
  ...
  const { job, task } = item;
  activeJob = job.id;
  ACTIVE_JOB_STARTED_AT = Date.now();
...
app.post('/queue/clear-stale', (req, res) => {
  if (activeJob && ACTIVE_JOB_STARTED_AT && Date.now() - ACTIVE_JOB_STARTED_AT > MAX_PROCESS_MS + 30000) {
    const oldJob = JOBS.get(activeJob);
    if (oldJob) {
      updateJob(oldJob, {
...
app.get('/status/:jobId', (req, res) => {
  ...
  res.json({
    ...
    message: job.status === 'queued' && pos > 1 ? `Aguardando finalizar o vídeo anterior` : job.message,
    logs: job.logs,
    result: job.result,
    error: job.error
  });
