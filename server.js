  ...
  try {
    ...
    if (!cuts.length) {
      return res.status(422).json({
        ok: false,
        code: 'NO_CUTS',
        error: 'Não encontramos silêncios suficientes para cortar nesse vídeo. Tente diminuir o volume de corte ou aumentar a sensibilidade.'
      });
    }
    ...
    res.status(400).json({ ok: false, code: err.code || 'PROCESS_ERROR', error: err.message || 'Erro ao processar vídeo.' });
  } finally {
...
app.use((err, req, res, next) => {
  ...
  if (err) return res.status(400).json({ ok: false, code: err.code || 'REQUEST_ERROR', error: err.message || 'Erro inesperado.' });
  next();
});
