...
async function detectSilences(inputPath, thresholdDb, minSilenceSec) {
  const { stderr } = await run('ffmpeg', [
    ...
    '-vn', '-af', 'silencedetect=n=' + thresholdDb + 'dB:d=' + minSilenceSec,
    '-f', 'null', '-'
  ]);
  const silences = [];
...
function buildSelectExpression(cuts) {
  if (!cuts.length) return '1';
  return 'not(' + cuts.map(c => 'between(t\,' + c.start.toFixed(3) + '\,' + c.end.toFixed(3) + ')').join('+') + ')';
}

async function renderWithCuts(inputPath, outputPath, cuts) {
  if (!cuts.length) {
    await run('ffmpeg', [
  ...
  const expr = buildSelectExpression(cuts);
  const scriptPath = path.join(ROOT_TMP, crypto.randomUUID() + '-filter.txt');
  const filter = `[0:v]select='${expr}',setpts=N/FRAME_RATE/TB[v];
...
app.post('/process', upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    await renderWithCuts(inputPath, outputPath, cuts);
