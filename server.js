...
const MAX_QUEUE_WAIT_MS = Number(process.env.MAX_QUEUE_WAIT_MS || 1000 * 60 * 5);
...
function antiCloneAndRateLimit(req, res, next) {
  ...
  const protectedRoute =
    ...
    req.path.startsWith('/cancel-queue');
...
async function renderPreciseSelect(inputPath, outputPath, cuts, duration) {
  ...
  for (let i = 0; i < keep.length; i++) {
    ...
    filter += `[0:v]trim=start=${start}:end=${end},setpts=PTS-STARTPTS,scale='if(gt(ih,${MAX_OUTPUT_HEIGHT}),-2,iw)':'if(gt(ih,${MAX_OUTPUT_HEIGHT}),${MAX_OUTPUT_HEIGHT},ih)'[v${i}];`;
...
function createJob(req) {
  ...
  const job = {
    id,
    ownerIp: req ? clientIp(req) : 'unknown',
    status: 'queued',
    progress: 1,
    message: 'Na fila...',
    logs: [],
...
function cleanupQueueState() {
  const now = Date.now();

  for (let i = PROCESS_QUEUE.length - 1; i >= 0; i--) {
    const queuedJob = PROCESS_QUEUE[i].job;
    if ((now - queuedJob.createdAt) > MAX_QUEUE_WAIT_MS) {
      updateJob(queuedJob, {
        status: 'error',
        progress: 100,
        message: 'Fila expirada',
        error: 'Esse processamento ficou muito tempo na fila. Tente novamente.'
      });
  ...
  if (activeJob) {
    ...
    const activeIsFinished = !job || job.status === 'done' || job.status === 'error';
    const activeIsStale = job && (now - job.updatedAt) > MAX_PROCESS_MS;
    if (activeIsFinished || activeIsStale) {
      if (job && activeIsStale) {
        updateJob(job, {
          status: 'error',
          progress: 100,
          message: 'Processamento liberado',
          error: 'O processamento anterior travou e foi liberado automaticamente.'
        });
      }
      activeJob = null;
    }
...
function removeDuplicateQueuedJobs(ownerIp) {
  for (let i = PROCESS_QUEUE.length - 1; i >= 0; i--) {
    const queuedJob = PROCESS_QUEUE[i].job;
    if (queuedJob.ownerIp === ownerIp && queuedJob.status === 'queued') {
      updateJob(queuedJob, {
        status: 'error',
        progress: 100,
        message: 'Substituído por novo envio',
        error: 'Um novo vídeo foi enviado. Este processamento antigo foi cancelado.'
      });
      PROCESS_QUEUE.splice(i, 1);
    }
  }
...
function enqueueJob(job, task) {
  cleanupQueueState();
  removeDuplicateQueuedJobs(job.ownerIp);
...
app.post('/cancel-queue', (req, res) => {
  const ip = clientIp(req);
  let cancelled = 0;

  for (let i = PROCESS_QUEUE.length - 1; i >= 0; i--) {
    const job = PROCESS_QUEUE[i].job;
    if (job.ownerIp === ip) {
      updateJob(job, {
        status: 'error',
        progress: 100,
