...
const MAX_OUTPUT_WIDTH = Number(process.env.MAX_OUTPUT_WIDTH || 1920);
...
function cancelOldProcessing(reason = 'Processamento anterior cancelado para iniciar um novo vídeo.') {
  // Uso pessoal no Render: evita ficar preso em “Na fila: 1 vídeo na frente”.
  for (const item of PROCESS_QUEUE.splice(0)) {
    updateJob(item.job, { status: 'error', progress: 100, message: 'Cancelado', error: reason });
    logJob(item.job, reason, 'warn');
  }

  if (activeJob) {
    const job = JOBS.get(activeJob);
    if (job) {
      updateJob(job, { status: 'error', progress: 100, message: 'Cancelado', error: reason });
      logJob(job, reason, 'warn');
    }
  }

  if (currentChildProcess) {
    try { currentChildProcess.kill('SIGKILL'); } catch (e) {}
...
app.post('/process', upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    // Ao enviar novo vídeo, limpa fila/job antigo preso.
    cancelOldProcessing();
...
app.get('/status/:jobId', (req, res) => {
  ...
  res.json({
    ...
    message: job.status === 'queued' ? 'Aguardando início do processamento...' : job.message,
    logs: job.logs,
    result: job.result,
    error: job.error
  });
