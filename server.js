...
async function renderWithCutsReencode(inputPath, outputPath, cuts, duration) {
  if (!cuts.length) {
    await run('ffmpeg', [
      ...
      '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '160k', '-threads', '0', outputPath
    ]);
    return;
  }
...
async function renderWithCutsCopy(inputPath, outputPath, cuts, duration) {
  ...
  // Modo TURBO: corta por stream copy. É muito mais rápido porque não reencoda o vídeo inteiro.
  // Em alguns vídeos, o corte pode grudar no keyframe mais próximo. Se o codec não permitir MP4, cai no modo compatível.
...
async function renderWithCuts(inputPath, outputPath, cuts, duration) {
  if (TURBO_COPY_MODE) {
    try {
      ...
      console.warn('Modo turbo falhou; usando modo compatível:', err.message);
...
app.get('/health', (req, res) => res.json({ ok: true, service: 'Silence Pro MP4 Smart HQ', ffmpeg: true, mode: 'quality_speed' }));
...
app.post('/process', upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      mode: TURBO_COPY_MODE ? 'Smart HQ: qualidade original quando possível, HQ rápido quando necessário' : 'HQ compatível'
    });
  } catch (err) {
