...
function startJob(job, task) {
  if (activeJob) {
    const busyError = new Error('Já existe um vídeo sendo processado. Aguarde terminar para enviar outro.');
...
app.post('/process', upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    if (isServerBusy()) {
      const busyError = new Error('Já existe um vídeo sendo processado. Aguarde terminar para enviar outro vídeo.');
      busyError.statusCode = 409;
      throw busyError;
    }
    ...
    const job = createJob();
    logJob(job, 'Upload recebido. Iniciando processamento...', 'success');

    startJob(job, async (job) => {
      let outputPath = null;
    ...
    if (inputPath) fsp.unlink(inputPath).catch(() => {});
    res.status(err.statusCode || 400).json({ ok: false, error: err.message || 'Erro ao iniciar processamento.' });
  }
});
