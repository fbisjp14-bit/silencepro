...
const FAST_COPY_MODE = String(process.env.FAST_COPY_MODE || '1') === '1';
  ...
  if (FAST_COPY_MODE) {
    try {
      ...
      if (outInfo && outInfo.duration <= Math.min(duration - 0.15, expected + 0.75)) {
        return { mode: 'turbo-copy-video-audio-fix' };
      }
...
function enqueueJob(job, task) {
  // Fila removida: o processamento começa imediatamente.
  // As rotas /process e /status continuam iguais para não alterar a estrutura do app.
  updateJob(job, {
    status: 'processing',
    progress: 10,
