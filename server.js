...
function jobLog(job, msg, type = 'info') {
  job.logs.push({ time: new Date().toLocaleTimeString('pt-PT', { hour12: false }), msg, type });
  if (job.logs.length > 80) job.logs.shift();
  job.updatedAt = Date.now();
}

async function processVideoToOutput(inputPath, originalName, threshold, minSilence, padding, job) {
  jobLog(job, 'A analisar vídeo...', 'info');
  updateJob(job, { status: 'processing', progress: 40, message: 'Analisando áudio...' });
  ...
  updateJob(job, { progress: 55, message: 'Detectando silêncios...' });
  ...
  jobLog(job, `Silêncios detectados: ${detected.silences.length + (detected.openSilenceStart !== null ? 1 : 0)}`, 'success');
  jobLog(job, `Cortes preparados: ${cuts.length}`, 'success');
  ...
  updateJob(job, { progress: 92, message: 'Finalizando arquivo...' });
  const outInfo = await probe(outputPath).catch(() => null);
...
app.post('/start-process', upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    const job = makeJob();
    updateJob(job, { status: 'processing', progress: 35, message: 'Upload concluído. Processando no servidor...' });
    jobLog(job, 'Upload concluído. O processamento continuará no servidor.', 'success');

    res.json({ ok: true, jobId: job.id });

    (async () => {
      try {
        const result = await processVideoToOutput(inputPath, req.file.originalname, threshold, minSilence, padding, job);
        updateJob(job, { ok: true, status: 'done', progress: 100, message: 'Processamento concluído!', result });
        jobLog(job, 'Pronto! Processamento concluído.', 'success');
      } catch (err) {
        updateJob(job, { ok: false, status: 'error', progress: 100, message: 'Erro no processamento', error: err.message || 'Erro ao processar vídeo.' });
        jobLog(job, 'ERRO CRÍTICO: ' + (err.message || 'Erro ao processar vídeo.'), 'error');
      } finally {
...
app.get('/job/:id', (req, res) => {
  ...
  if (!job) {
    res.status(404).json({ ok: false, error: 'Processamento não encontrado ou expirado.' });
    return;
  }
  res.json({
    ok: true,
