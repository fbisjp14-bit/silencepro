...
async function renderCopyConcat(inputPath, outputPath, cuts, duration) {
  ...
  try {
    // Relâmpago seguro:
    // mantém o VÍDEO em copy para preservar velocidade/qualidade,
    // mas RECODIFICA apenas o ÁUDIO para corrigir repetição/gagueira causada por timestamps/keyframes.
    await run('ffmpeg', [
...
async function renderWithCuts(inputPath, outputPath, cuts, duration, outputMode = 'turbo') {
  ...
  if (settings.mode === 'turbo') {
    ...
    try {
      await renderCopyConcat(inputPath, outputPath, cuts, duration);
      return { mode: 'relampago-video-copy-audio-fix', outputMode: 'Relâmpago Seguro' };
    } catch (e) {
      fsp.unlink(outputPath).catch(() => {});
