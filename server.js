...
async function mergeProcessedVideos(fileNames, outputPath) {
  const names = (Array.isArray(fileNames) ? fileNames : []).map(name => path.basename(String(name || ''))).filter(Boolean);
  if (!names.length) throw new Error('Nenhuma parte processada foi enviada para unir.');

  const inputPaths = names.map(name => path.join(OUTPUT_DIR, name));
  for (const filePath of inputPaths) {
    if (!fs.existsSync(filePath)) throw new Error('Uma das partes expirou ou não existe mais. Processe novamente: ' + path.basename(filePath));
  }

  if (inputPaths.length === 1) {
    await fsp.copyFile(inputPaths[0], outputPath);
    const info = await probe(outputPath).catch(() => null);
    return { mode: 'single-copy', seconds: info && info.duration ? info.duration : 0 };
  }

  const listPath = path.join(ROOT_TMP, crypto.randomUUID() + '-merge.ffconcat');
  let list = 'ffconcat version 1.0\n';
  for (const filePath of inputPaths) list += `file '${quoteConcatPath(filePath)}'\n`;
  await fsp.writeFile(listPath, list, 'utf8');
  ...
  try {
    try {
      await run('ffmpeg', [
        ...
        '-f', 'concat', '-safe', '0', '-i', listPath,
        '-map', '0:v:0', '-map', '0:a:0?',
        '-c', 'copy', '-movflags', '+faststart', '-fflags', '+genpts', outputPath
      ]);
      const info = await probe(outputPath).catch(() => null);
      return { mode: 'concat-copy', seconds: info && info.duration ? info.duration : 0 };
    } catch (copyErr) {
      fsp.unlink(outputPath).catch(() => {});
      await run('ffmpeg', [
        '-hide_banner', '-y', '-nostdin',
      ...
      const info = await probe(outputPath).catch(() => null);
      return { mode: 'concat-reencode-fallback', seconds: info && info.duration ? info.duration : 0 };
    }
  } finally {
    fsp.unlink(listPath).catch(() => {});
  }
...
function isProcessSlotBusy() {
  return processSlotLocked || hasRunningJob();
}
...
function processJobNow(job, task) {
  activeJobId = job.id;
  processSlotLocked = true;
  setImmediate(async () => {
    try {
      updateJob(job, { status: 'processing', progress: 22, message: 'Iniciando processamento...' });
...
app.post('/process', (req, res) => {
  if (isProcessSlotBusy()) {
    return res.status(429).json({
      ok: false,
      error: 'Aguarde o vídeo atual terminar antes de enviar outro.'
    });
  }
  ...
  upload.single('video')(req, res, async (uploadErr) => {
    ...
    try {
      ...
      processJobNow(job, async (job) => {
        ...
        try {
          updateJob(job, { status: 'processing', progress: 30, message: 'Analisando vídeo...' });
          ...
          updateJob(job, { progress: 45, message: 'Detectando silêncios...' });
          logJob(job, 'Detectando silêncios...', 'info');
          const detected = await detectSilences(inputPath, threshold, minSilence);

          updateJob(job, { progress: 60, message: 'Preparando cortes...' });
          ...
          updateJob(job, { progress: 75, message: 'Renderizando vídeo...' });
...
app.post('/merge', (req, res) => {
  if (isProcessSlotBusy()) {
    return res.status(429).json({
      ok: false,
      error: 'Aguarde o processamento atual terminar antes de unir as partes.'
    });
  }
  ...
  try {
    ...
    if (files.length < 2) throw new Error('Envie pelo menos 2 partes processadas para unir.');

    const baseName = safeBaseName((req.body && req.body.baseName) || 'video_partes');
    const job = createJob();
    logJob(job, 'Partes recebidas. Preparando união final...', 'success');

    processJobNow(job, async (job) => {
      let outputPath = null;
      try {
        updateJob(job, { progress: 25, message: 'Verificando partes...' });
        logJob(job, `Partes para unir: ${files.length}`, 'info');

        updateJob(job, { progress: 55, message: 'Unindo partes...' });
        const outputName = 'SilencePro_' + baseName + '_FINAL_' + crypto.randomUUID().slice(0, 8) + '.mp4';
        outputPath = path.join(OUTPUT_DIR, outputName);
        const mergeResult = await mergeProcessedVideos(files, outputPath);

        updateJob(job, { progress: 92, message: 'Finalizando MP4 final...' });
        const outInfo = await probe(outputPath).catch(() => null);
        const finalSeconds = outInfo && outInfo.duration ? outInfo.duration : (mergeResult.seconds || 0);

        logJob(job, 'MP4 final unido com sucesso.', 'success');
        return {
          ok: true,
          fileName: outputName,
          downloadUrl: '/download/' + encodeURIComponent(outputName),
          ...
          mergeMode: mergeResult.mode || 'concat'
        };
      } catch (err) {
        if (outputPath) fsp.unlink(outputPath).catch(() => {});
        throw err;
    ...
    processSlotLocked = false;
    res.status(400).json({ ok: false, error: err.message || 'Erro ao iniciar união das partes.' });
  }
});
