...
const VIDEO_CRF = String(process.env.VIDEO_CRF || '20');
...
async function renderSegments(inputPath, outputPath, segments) {
  ...
  const script = [
    "[0:v:0]select='" + expr + "',setpts=N/FRAME_RATE/TB[outv]",
    "[0:a:0]aselect='" + expr + "',asetpts=N/SR/TB[outa]"
  ].join(';\n');
  ...
  try {
    ...
    if ((err.stderr || '').includes('Error initializing complex filters') || (err.stderr || '').includes('Invalid argument')) {
      await renderSegmentsFallback(inputPath, outputPath, segments);
      return;
    }
    throw err;
  } finally {
...
async function renderSegmentsFallback(inputPath, outputPath, segments) {
