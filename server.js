...
function nextKeyframeAtOrAfter(keyframes, value, duration) {
  const t = Math.max(0, Number(value) || 0);
  for (const k of keyframes) {
    if (k >= t - 0.001) return Math.min(duration, k);
  }
  return Math.min(duration, t);
}

function previousKeyframeAtOrBefore(keyframes, value, duration) {
...
function makeKeyframeSafeKeepRanges(keep, keyframes, duration) {
  if (!keyframes.length) return keep;
  ...
  for (let i = 0; i < keep.length; i++) {
    ...
    // Para não repetir cena/áudio em copy mode, o começo do trecho precisa iniciar em keyframe.
    // Em vez de voltar para keyframe anterior, avançamos para o próximo keyframe.
    // Isso evita a repetição/travada que você viu.
    const start = i === 0 && r.start <= 0.05
      ? 0
      : nextKeyframeAtOrAfter(keyframes, r.start, total);

    // O fim também fica em ponto seguro quando possível.
    // No último trecho mantemos o final real para não cortar o final do vídeo.
    let end = i === keep.length - 1
      ? Math.min(total, r.end)
      : previousKeyframeAtOrBefore(keyframes, r.end, total);

    if (end <= start + 0.08) {
      // Se os keyframes ficarem apertados demais, usa o fim original para não apagar tudo.
      end = Math.min(total, r.end);
    }

    if (end > start + 0.08) safe.push({ start, end });
  }

  return safe.length ? safe : keep;
}


async function renderCopyConcat(inputPath, outputPath, cuts, duration) {
  let keep = buildKeepRanges(duration, cuts);
  ...
  keep = makeKeyframeSafeKeepRanges(keep, keyframes, duration);
  ...
  try {
    // Relâmpago sem repetição:
    // 1) ajusta os pontos de corte para keyframes seguros;
    // 2) mantém vídeo em copy para preservar velocidade;
    // 3) recodifica só o áudio para corrigir timestamps e reduzir gagueira.
    await run('ffmpeg', [
...
async function renderWithCuts(inputPath, outputPath, cuts, duration, outputMode = 'turbo') {
  ...
  if (settings.mode === 'turbo') {
    // Modo Relâmpago real:
    // corta por cópia, sem reencodar o vídeo inteiro.
    // É muito mais rápido no Render e mantém a qualidade original.
    // Se falhar por causa do arquivo, cai para o Ultra Turbo com reencode leve.
    try {
      await renderCopyConcat(inputPath, outputPath, cuts, duration);
      return { mode: 'relampago-keyframe-safe-audio-fix', outputMode: 'Relâmpago Sem Repetir' };
    } catch (e) {
      fsp.unlink(outputPath).catch(() => {});
      await renderTurboSelect(inputPath, outputPath, cuts, duration, outputMode);
      return { mode: 'ultra-turbo-select-fallback', outputMode: settings.label };
    }
  }
