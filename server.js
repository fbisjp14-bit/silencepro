...
const TRANSCRIPT_DIR = path.join(ROOT_TMP, 'transcripts');
...
const TRANSCRIBE_MAX_MS = Number(process.env.TRANSCRIBE_MAX_MS || 1000 * 60 * 20);
...
function cleanOldFiles() {
  ...
  for (const dir of [UPLOAD_DIR, OUTPUT_DIR, TRANSCRIPT_DIR]) {
...
async function drainQueue() {
  ...
  try {
    ...
    logJob(job, 'A ligar o motor de processamento...', 'info');
...
function validatePublicHttpUrl(raw) {
  ...
  if (!value) throw new Error('Cole um link de vídeo primeiro.');
  if (value.length > 2048) throw new Error('Link muito grande.');
  let parsed;
  try { parsed = new URL(value); } catch (e) { throw new Error('Link inválido. Cole uma URL completa começando com http ou https.'); }
  if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error('Use apenas links http ou https.');
  const host = parsed.hostname.toLowerCase();
  if (host === 'localhost' || host.endsWith('.localhost') || host === '0.0.0.0' || host.startsWith('127.') || host.startsWith('10.') || host.startsWith('192.168.')) {
    throw new Error('Por segurança, use apenas links públicos de vídeo.');
  }
  const private172 = host.match(/^172\.(1[6-9]|2\d|3[0-1])\./);
  if (private172) throw new Error('Por segurança, use apenas links públicos de vídeo.');
  return parsed.toString();
}

function parseTranscriptJson(stdout) {
  const text = String(stdout || '').trim();
  if (!text) throw new Error('O motor de transcrição não retornou resposta.');
...
app.post('/transcribe-link', async (req, res) => {
  try {
    ...
    res.status(400).json({ ok: false, error: err.message || 'Erro ao iniciar transcrição.' });
  }
});
...
app.get('/transcript/:file', (req, res) => {
  const file = path.basename(req.params.file);
  const full = path.join(TRANSCRIPT_DIR, file);
  if (!fs.existsSync(full)) return res.status(404).send('Transcrição expirou. Gere novamente.');
  res.setHeader('Cache-Control', 'no-store');
  res.download(full, file);
});
